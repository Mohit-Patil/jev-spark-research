...
def main():
    out=ROOT/'counterfactual_trial_v3';out.mkdir(exist_ok=False);before=Budget().count()
    ...
    frozen={'kind':'single-boundary observed-state-matched randomized counterfactual trial','frozen_unix':time.time(),
      'case':c,'target_signature':anchor,'target_snapshot':target,'arms':['CURRENT','PROPOSED'],'seed':1846,
      'calibration_runs':1,'warmup_rounds':2,'measured_rounds':10,'maximum_overrides_per_query':1,
    ...
    spark.sparkContext.setLogLevel('ERROR');rows=[];rng=random.Random(1846)
    try:
        ...
        for phase,reps in [('warmup',2),('measured',10)]:
            for rep in range(reps):
                arms=['CURRENT','PROPOSED'];rng.shuffle(arms)
                for position,arm in enumerate(arms):run(phase,rep,arm,position)
        result={'status':'succeeded','target_signature':anchor,'runs':len(rows),'measured_runs':20,'all_correct':True,
