...
from authorized_budget import AuthorizedBudget as Budget
...
def main():
    ...
    for d in decisions:
        ...
        rebuilt=rebuild_request(d['snapshot'])
    ...
    report={'status':'succeeded','audit_unix':time.time(),'scope':'production_v4 experiments only; inherited practical_v2 excluded',
        ...
        'new_api_attempts_in_this_continuation':4,'api_attempts_consumed_by_audit':0,'global_budget_used':after,'global_limit':Budget().status()['limit'],'global_remaining':Budget().status()['limit']-after,
    ...
    with (R/'AUDIT_COMPLETED_V2.json').open('w') as f:json.dump(report,f,indent=2)
    with (R/'AUDIT_CHECKS_V2.json').open('w') as f:json.dump(checks,f,indent=2)
