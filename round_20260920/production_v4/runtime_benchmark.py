...
ARMS=['OFF','SHADOW','APPLY','PRE_BROADCAST']
CASES=[dict(name='left_skew12m',n=12000000,m=2000000,keep=500,skew=True,seed=313,join='LEFT'),
       dict(name='left_uniform12m',n=12000000,m=2000000,keep=500,skew=False,seed=313,join='LEFT')]
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--label',default='runtime_dev_v1');ap.add_argument('--reps',type=int,default=5)
    ...
    manifest={'cases':CASES,'arms':ARMS,'warmups':2,'reps':args.reps,'seed':90267,'scope':'runtime fixed-rule development, no model-driven choice',
      ...
      'holdout':'new runtime families not yet evaluated; these are development cases'}
            ...
            for phase,reps in [('warmup',2),('measured',args.reps)]:
                for rep in range(reps):
                    ...
                    for pos,arm in enumerate(arms):
                        ...
                        spark.conf.set('spark.research.v4.mode','OFF' if arm=='PRE_BROADCAST' else arm)
                        ...
                        r.update(arm=arm,phase=phase,rep=rep,position=pos,events=new,
