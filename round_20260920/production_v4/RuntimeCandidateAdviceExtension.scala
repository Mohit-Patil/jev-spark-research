...
/** Disposable Spark 4.0.1-only experiment. Loopback callback only; no remote endpoint or credential in the JVM.
 ...
 * This is candidate-generation research, with a separately gated experimental Jev policy.
 */
class RuntimeCandidateAdviceExtension extends (SparkSessionExtensions => Unit) {
  override def apply(e: SparkSessionExtensions): Unit = {
    e.injectQueryStagePrepRule((s: SparkSession) => new RuntimeCandidateAdviceRule(s))
  }
}
class RuntimeCandidateAdviceRule(session: SparkSession) extends Rule[SparkPlan] {
  ...
  override def apply(input:SparkPlan):SparkPlan = {
    val mode=SQLConf.get.getConfString("spark.research.v5.mode","OFF")
    if(mode=="OFF") return input
    val started=System.nanoTime()
    try {
      require(org.apache.spark.SPARK_VERSION=="4.0.1","unsupported version")
      require(Set("SHADOW","APPLY","JEV").contains(mode),"unsupported mode")
      val tag=SQLConf.get.getConfString("spark.research.v5.run_id","unlabeled")
          ...
          (leaf(j.left),leaf(j.right)) match {
            case (Some((l,lq)),Some((r,rq))) if lq.isMaterialized && rq.isMaterialized =>
              ...
              if(!allowed) j else {
                ...
                if(select) prepared else j
              }
            case _ => j
          }
      }
      if(changed) {
        val finalPlan=if(didApply) EnsureRequirements().apply(result) else input
        ...
        if(mode=="APPLY" || mode=="JEV") {
          used.add(tag);if(used.size>128)used.remove(used.head)
        }
        RuntimeCandidateAdviceEvents.record(saved)
        finalPlan
      } else input
    } catch {
      case NonFatal(e) => RuntimeCandidateAdviceEvents.error(e.getClass.getSimpleName);input
    } finally {RuntimeCandidateAdviceEvents.addTime(System.nanoTime()-started)}
  }
}
object RuntimeCandidateAdviceEvents {
  private val events=mutable.ArrayBuffer.empty[String]
  private val errors=mutable.ArrayBuffer.empty[String]
  private val mapper=new ObjectMapper()
  private var nanos=0L
  /** Only a bounded loopback service; no remote endpoint or credential in the JVM. */
