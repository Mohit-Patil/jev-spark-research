"""Checkpoint only this reviewed production_v3 iteration. No model calls.
Do not stage another task's work, data files, binaries, credentials or request ledger.
"""
...
def main():
    ...
    if cmd(['git','diff','--cached','--name-only']):raise RuntimeError('other_staged_work_present')
    key,metadata=load_key();files=[]
    for f in R.rglob('*'):
        ...
        if f.name=='last_git_checkpoint.json':continue
        if f.stat().st_size>4000000:raise RuntimeError('artifact_review_size_bound')
        if not scan(f.read_bytes(),key):raise RuntimeError('secret_scan_failed')
        files.append(str(f.relative_to(WS)))
    key=None
    cmd(['git','check-ignore','--no-index','round_20260920/production_v3/.env'])
    cmd(['git','add','--']+sorted(files))
    staged=cmd(['git','diff','--cached','--name-only']).splitlines()
    if any(not f.startswith('round_20260920/production_v3/') for f in staged):
        raise RuntimeError('concurrent_staging_detected_stop')
    changed=bool(staged)
    ...
    report={'commit':cmd(['git','rev-parse','HEAD']),'commit_created':changed,'files_scanned':len(files),
      'scope':'round_20260920/production_v3 only','secret_scan_passed':True,'env_ignored':True,
