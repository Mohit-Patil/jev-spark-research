...
def main():
    ...
    runtime=HERE/'runtime_safety_v1'/'COMPLETED.json'
    ...
    lines+=['','The wide-small-probe case is a counterexample to unconditional broadcasting: native sort-merge beat forced broadcast in every measured round. On the wide-large-probe case, broadcast essentially tied native despite a roughly 3.4-second query duration. More runtime alone does not create optimization headroom. Small differences between arms with the same final operator are not treated as proven effects of configuration.','',
      ...
      'Runtime/loopback integration status: '+report['runtime_safety']['status']+'. The prepared integration script tests native fallback for both CURRENT and PROPOSED, malformed and stale replies, a slow-drip response with a 150 ms overall async deadline, and two Spark pass-through/audit-capacity queries. Only completed checks in AUDIT.json count as executed.','',
      ...
      'The Mac worker returned a busy response to attempted safety-integration launches; another tracked research job was not interrupted. The status above records whether a later retry completed. No unattended continuation was scheduled by this iteration.','',
