 ...
 */
class NativeBoundaryObserverV6(conf: SparkConf) extends CostEvaluator {
  ...
  private def stateSignature(current:JMap[String,Object], proposed:JMap[String,Object]):String = {
    ...
    val bytes=NativeBoundaryObserverV6.canonicalMapper.writeValueAsBytes(state)
  ...
  override def evaluateCost(plan: SparkPlan): Cost = synchronized {
    ...
    NativeBoundaryObserverV6.calls.incrementAndGet()
    try {
      ...
      site match {
        case Some(s) if s.getLineNumber==365 =>
          if(pending.contains(tid)) NativeBoundaryObserverV6.errors.incrementAndGet()
        ...
        case Some(s) if s.getLineNumber==366 =>
          pending.remove(tid) match {
            case Some((current,oldCost,oldSnapshot,oldTag)) if oldTag==tag =>
              ...
              val selected=eligible && NativeBoundaryObserverV6.claim(tag)
            ...
            case _ => NativeBoundaryObserverV6.errors.incrementAndGet()
          }
        case _ => NativeBoundaryObserverV6.unmatched.incrementAndGet()
      }
    } catch {case NonFatal(e)=>returned=stock;NativeBoundaryObserverV6.errors.incrementAndGet()}
    finally NativeBoundaryObserverV6.observerNanos.addAndGet(System.nanoTime()-begin)
    returned
  }
}
object NativeBoundaryObserverV6 {
  ...
  def completeSnapshot(m:JMap[String,Object]):Boolean = {
