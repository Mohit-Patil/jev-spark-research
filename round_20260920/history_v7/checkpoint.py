"""Secret-scanned commit/push of this iteration only. No model requests.
The credential is only read locally for exact-match scanning, never returned.
"""
...
def main():
    ...
    for f in R.rglob('*'):
        ...
        if f.name=='last_git_checkpoint.json' or f.suffix not in ('.py','.scala','.json','.jsonl','.md','.txt'):continue
    ...
    if any(not f.startswith('round_20260920/history_v7/') for f in staged):raise RuntimeError('concurrent_staging_stop')
    ...
    report={'commit':cmd(['git','rev-parse','HEAD']),'commit_created':bool(staged),'files_scanned':len(files),
      'scope':'history_v7 only','secret_scan_passed':True,'env_ignored':True,'budget':AuthorizedBudget().status(),'push_succeeded':False}
    ...
    (R/'last_git_checkpoint.json').write_text(json.dumps(report,indent=2));print(json.dumps(report,indent=2))
